...
class YourCustomPlayer2(Player):
    @property
    def name(self):
...
def min_max(state, current_depth, trans):
    ...
    if current_depth == 0:
        ...
        return evaluate(state, state.to_play.color)
    ...
    for i in range(0, len(state.board)):
        for j in range(0, len(state.board[0])):
            if state.board[i][j] == color:
                count = count + 1
                chase_list.append(chase(i, j, state, color))
    chase_list.sort()
...
def chase(i, j, state, color):
    ...
    if j == 0 or i == len(state.board) - 1 or j>0 and i < len(state.board) - 1 and state.board[i+1][j-1] != color:
        ...
        while ii >= 0 and jj < len(state.board[0]) and state.board[ii][jj] == color:
            ...
            jj = jj + 1
        return_list.append(count)
    if not return_list: return return_list[len(return_list)-1]
    return 0
